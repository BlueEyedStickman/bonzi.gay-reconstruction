...
// Vault endpoint for the vault feature
app.post('/vault', (req, res) => {
  // Simple vault implementation
  const { guess, tag } = req.body;
  
  // You can implement your own vault logic here
    ...
    switch (cmd) {
      ...
      case 'godmode':
        ...
        if (rooms[room][guid]) {
          ...
          io.to(room).emit('update', { guid, userPublic: rooms[room][guid] });
          // Send admin status to the client with proper data structure
          socket.emit('admin', { admin: true });
        }
        break;
      case 'king':
        // Check password for king command
        if (!args[0]) {
          socket.emit('alert', { text: 'Did you try password?' });
          break;
        }
        if (args[0] !== config.king_password) {
          socket.emit('alert', { text: 'Did you try password?' });
          break;
        }
      ...
      case 'vid':
        ...
        break;
      case 'advpoll':
        // Advanced poll creation: format is "title;option1;option2;..."
      ...
      case 'hat':
        if (args[0]) {
          userPublic.hat = args[0].substring(0, 32);
          io.to(room).emit('update', { guid, userPublic });
        }
        break;
      case 'quote':
        // Quote command - args[0] is target name, rest is the quoted message
        io.to(room).emit('quote', { 
          guid, 
          target: args[0] || '', 
      ...
      case 'gif':
        if (args[0]) {
          io.to(room).emit('gif', { guid, url: args[0] });
        }
        break;
      case 'pawn':
        io.to(room).emit('pawn', { guid });
        break;
      case 'insult':
        io.to(room).emit('insult', { guid, target: args[0] || '' });
        break;
    }
  });
